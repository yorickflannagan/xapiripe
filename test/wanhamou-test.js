...
class LogTest
{
	...
	logRotationTestCase() {
		...
		let file = path.join(__dirname, '..', 'components', Wanhamou.Logger.cfgLogPattern.replace('-n', '-1'));
		if (!fs.existsSync(file)) {
			Wanhamou.Logger.releaseLogger();
	...
	logTruncationTestCase() {
		...
		let file = path.join(__dirname, '..', 'components', Wanhamou.Logger.cfgLogPattern.replace('-n', '-2'));
		if (!fs.existsSync(file)) {
			Wanhamou.Logger.releaseLogger();
			throw new Error('Rotated log file not found in the file system');
		}
